    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            Button {
                ...
                Label(
                    watchedAll ? "Mark as unseen" : "Mark as seen",
                    systemImage: watchedAll ? "eye.fill" : "eye"
                )
                .frame(height: 30)
            ...
            if let tmdbEpisodes = tvSeason.episodes {
                ...
                ForEach(tmdbEpisodes) { tvEpisode in
                    TVEpisodeRow(
                        tvShow: tvShow,
                        tvEpisode: tvEpisode,
                        tvBuddyTVEpisode: tvBuddyTVEpisodes.first(where: { $0.id == tvEpisode.id })
                    )
                }
            }
        }
        .task(id: tvBuddyTVShow?.episodes) {
            ...
            do {
                tvBuddyTVEpisodes = try context.fetch(
                    FetchDescriptor<TVBuddyTVEpisode>(
                        predicate: #Predicate<TVBuddyTVEpisode> {
                            $0.tvShow?.id == id && $0.seasonNumber == seasonNumber
                        }
                    )
                )
            } catch {
